...
impl StructType {
    ...
    pub fn get_elements(&self) -> Vec<&Type> {
        unsafe {
            ...
            let mut els: Vec<LLVMTypeRef> = (0..size)
                .map(|_| MaybeUninit::zeroed().assume_init())
                .collect();
            core::LLVMGetStructElementTypes(self.into(), els.as_mut_ptr() as *mut LLVMTypeRef);
            els.iter().map(|&el_ptr| mem::transmute(el_ptr)).collect()
        }
    }
}
...
impl FunctionType {
    ...
    pub fn get_params(&self) -> Vec<&Type> {
        unsafe {
            ...
            let mut types: Vec<LLVMTypeRef> = (0..count)
                .map(|_| MaybeUninit::zeroed().assume_init())
                .collect();
            core::LLVMGetParamTypes(self.into(), types.as_mut_ptr() as *mut LLVMTypeRef);
            types.iter().map(|t| mem::transmute(t)).collect()
        }
    }
