...
use ffi::prelude::{LLVMAttributeRef, LLVMValueRef};
use ffi::{core, LLVMIntPredicate, LLVMLinkage, LLVMRealPredicate};
    ...
    ($this:ty, $name:ident, $sup:ty) => {
        unsafe impl Sub<$sup> for $this {
            fn is(value: &$sup) -> bool {
                unsafe { !core::$name(value.into()).is_null() }
            }
...
impl Value {
    ...
    pub fn new_undef(ty: &Type) -> &Value {
    ...
    pub fn get_name(&self) -> Option<&str> {
        unsafe {
            let mut len = 0;
            let c_name = core::LLVMGetValueName2(self.into(), &mut len) as *const u8;
    ...
    pub fn set_name(&self, name: &str) {
        ...
        unsafe {
            core::LLVMSetValueName2(self.into(), c_name.as_ptr(), c_name.as_bytes().len() - 1)
        }
    }
    ...
    pub fn is_constant(&self) -> bool {
        unsafe { core::LLVMIsConstant(self.into()) == 1 }
    }

    pub fn replace_all_uses(old: &Value, new: &Value) {
        unsafe { core::LLVMReplaceAllUsesWith(old.into(), new.into()) }
    }
}
...
impl Into<LLVMIntPredicate> for Predicate {
...
impl Into<LLVMRealPredicate> for Predicate {
...
pub struct Param(PhantomData<[u8]>);
native_ref!(&Param = LLVMValueRef);
sub! {Param, LLVMIsAArgument}
to_str! {Param, LLVMPrintValueToString}
impl Param {
    ...
    pub fn set_alignment(&self, alignment: usize) {
        unsafe {
            ffi::core::LLVMSetAlignment(self.into(), alignment as libc::c_uint);
        }
    }
}
...
impl Function {
    ...
    pub fn params(&self) -> Box<dyn Iterator<Item = &Param> + '_> {
        Box::new((0..self.num_params()).map(move |i| &self[i]))
    }
...
pub struct Attribute(PhantomData<[u8]>);
